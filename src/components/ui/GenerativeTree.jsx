...
import { generativeTreeSource } from "./generative-tree-utils/generative-tree-source";
...
function buildFocusedDocument(size, particleAmount) {
...
html, body, canvas { width: 100%; height: 100%; margin: 0; overflow: hidden; background: #000000; }
...
export default function GenerativeTree({
  ...
  const source = useMemo(
    () => buildFocusedDocument(size, particleAmount),
    [particleAmount, size],
  );
  ...
  return (
    <div
      className={`threeui-background generative-tree absolute inset-0 w-full h-full pointer-events-none ${className ? ` ${className}` : ""}`}
